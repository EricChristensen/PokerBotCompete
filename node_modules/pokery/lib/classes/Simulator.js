...
export default class Simulator {
  ...
  run(n = 1000) {
    if (this.board.length) {
      console.log(`Running ${this.ranges.join(' vs. ')} on ${this.board} board` + `, ${n} times`);
    } else {
      console.log(`Running ${this.ranges.join(' vs. ')}, ${n} times`);
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      let board = this.board.concat(deck.draw(c.BOARD_LENGTH - this.board.length));
    ...
    if (this.failedRuns) {
      console.log(`${this.failedRuns} failed runs due to hole card ` + 'collisions.');
    }
