...
describe('Hand.vs', () => {
  it('compares high card vs. better high card', () => {
    assert.equal(new Hand(['Ac', 'Td', '7h', '5s', '2c']).vs(new Hand(['As', 'Th', '7d', '5c', '3s'])), -1);
  });

  it('compares high card vs. same high card', () => {
    assert.equal(new Hand(['Ac', 'Td', '7h', '5s', '3c']).vs(new Hand(['As', 'Th', '7d', '5c', '3s'])), 0);
  });

  it('compares high card vs. worse high card', () => {
    assert.equal(new Hand(['Ac', 'Td', '7h', '5s', '3c']).vs(new Hand(['As', '9h', '7d', '5c', '3s'])), 1);
  });

  it('compares high card vs. pair', () => {
    assert.equal(new Hand(['Ac', 'Td', '7h', '5s', '3c']).vs(new Hand(['As', 'Ah', '7d', '5c', '3s'])), -1);
  });

  it('compares pair vs. pair with better kickers', () => {
    assert.equal(new Hand(['Ac', 'Ad', '7h', '5s', '2c']).vs(new Hand(['As', 'Ah', '7d', '5c', '3s'])), -1);
  });

  it('compares pair vs. same pair', () => {
    assert.equal(new Hand(['Kc', 'Kd', '7h', '5s', '2c']).vs(new Hand(['Ks', 'Kh', '7d', '5c', '2s'])), 0);
  });

  it('compares pair vs. worse pair', () => {
    assert.equal(new Hand(['Ac', 'Ad', '7h', '5s', '2c']).vs(new Hand(['Js', 'Jh', '7d', '5c', '3s'])), 1);
  });

  it('compares two pair vs. better two pair', () => {
    assert.equal(new Hand(['Tc', 'Td', '5h', '5s', 'Ac']).vs(new Hand(['Js', 'Jh', '3d', '3c', '2s'])), -1);
  });

  it('compares two pair vs. same two pair', () => {
    assert.equal(new Hand(['4c', '4d', '3h', '3s', 'Ac']).vs(new Hand(['4s', '4h', '3d', '3c', 'As'])), 0);
  });

  it('compares two pair vs. two pair with worse kicker', () => {
    assert.equal(new Hand(['Tc', 'Td', '5h', '5s', 'Ac']).vs(new Hand(['Ts', 'Th', '5d', '5c', 'Ks'])), 1);
  });

  it('compares trips vs. better trips', () => {
    assert.equal(new Hand(['Tc', 'Td', 'Th', 'As', 'Kc']).vs(new Hand(['Js', 'Jh', 'Jd', '3c', '2s'])), -1);
  });

  it('compares trips vs. worse trips', () => {
    assert.equal(new Hand(['3c', '3d', '3h', '4s', '5c']).vs(new Hand(['2s', '2h', '2d', 'Qc', 'Js'])), 1);
  });

  it('compares straight vs. better straight', () => {
    assert.equal(new Hand(['5c', '6d', '7h', '8s', '9c']).vs(new Hand(['6s', '7h', '8d', '9c', 'Ts'])), -1);
  });

  it('compares straight vs. same straight', () => {
    assert.equal(new Hand(['6c', '7d', '8h', '9s', 'Tc']).vs(new Hand(['6c', '7d', '8h', '9h', 'Td'])), 0);
  });

  it('compares straight vs. worse straight', () => {
    assert.equal(new Hand(['Ts', 'Jh', 'Qd', 'Kc', 'As']).vs(new Hand(['9c', 'Td', 'Jh', 'Qs', 'Kc'])), 1);
  });

  it('compares flush vs. better flush', () => {
    assert.equal(new Hand(['Ac', 'Tc', '7c', '5c', '2c']).vs(new Hand(['Ac', 'Tc', '7c', '5c', '3c'])), -1);
  });

  it('compares flush vs. same flush', () => {
    assert.equal(new Hand(['Ah', 'Th', '7h', '5h', '3h']).vs(new Hand(['Ah', 'Th', '7h', '5h', '3h'])), 0);
  });

  it('compares flush vs. worse flush', () => {
    assert.equal(new Hand(['As', 'Ts', '7s', '5s', '3s']).vs(new Hand(['As', '9s', '7s', '5s', '3s'])), 1);
  });

  it('compares full house vs. better full house', () => {
    assert.equal(new Hand(['Ac', 'Ad', 'Ah', '2s', '2c']).vs(new Hand(['Ac', 'Ad', 'Ah', '7c', '7s'])), -1);
  });

  it('compares full house vs. same full house', () => {
    assert.equal(new Hand(['Ac', 'Ad', 'Ah', '2s', '2c']).vs(new Hand(['Ac', 'Ad', 'Ah', '2c', '2s'])), 0);
  });

  it('compares full house vs. worse full house', () => {
    assert.equal(new Hand(['Ac', 'Ad', 'Ah', '2s', '2c']).vs(new Hand(['Ks', 'Kh', 'Kd', 'Qc', 'Qs'])), 1);
  });

  it('compares quads vs. better quads', () => {
    assert.equal(new Hand(['Kc', 'Kd', 'Kh', 'Ks', 'As']).vs(new Hand(['Ac', 'Ad', 'Ah', 'As', 'Ks'])), -1);
  });

  it('compares quads vs. same quads', () => {
    assert.equal(new Hand(['Kc', 'Kd', 'Kh', 'Ks', 'As']).vs(new Hand(['Kc', 'Kd', 'Kh', 'Ks', 'As'])), 0);
  });

  it('compares quads vs. worse quads', () => {
    assert.equal(new Hand(['Ac', 'Ad', 'Ah', 'As', 'Jc']).vs(new Hand(['Ac', 'Ad', 'Ah', 'As', 'Ts'])), 1);
  });
});